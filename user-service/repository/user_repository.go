...
// USER ROLE: Assign User to role, GET User Role by id, Get All user Role, edit Assign user to role
...
// Assign UserToRole implements UserRepositoryInterface
...
func (u *userRepository) CreateUser(ctx context.Context, user model.User) (*model.User, error) {
	...
	if user.ID == 0 {
		...
		return nil, errors.New("User ID is Invalid after created")
	}
...
func (u *userRepository) GetAllUsers(ctx context.Context, page, limit int, search, sortBy, sortOrder string) ([]model.User, int64, error) {
	...
	Find(&modelUsers).Error; err != nil {
		log.Errorf("[Repository] GetAllUsers - 3: %v", err)
		return nil, 0, err
	}
	return modelUsers, totalRecord, nil
...
func (u *userRepository) GetUserByEmail(ctx context.Context, email string) (*model.User, error) {
	...
	First(&modelUsers).Error; err != nil {
		log.Errorf("[Repository] GetUserByEmail - 2: %v", err)
		return nil, err
	}
...
func (u *userRepository) GetUserByID(ctx context.Context, id uint) (*model.User, error) {
	...
	Preload("Roles").
	First(&modelUsers).Error; err != nil {
		log.Errorf("[Repository] GetUserByID - 2: %v", err)
		return nil, err
	}
	return &modelUsers, nil
}
...
func (u *userRepository) GetUserByRoleName(ctx context.Context, roleName string) ([]model.User, error) {
	...
	// Gunakan subquery untuk mendapatkan user IDs yang memiiki role tertentu
